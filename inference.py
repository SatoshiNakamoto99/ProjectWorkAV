...
class ModelInference():
    def __init__(self, model_path, images_path) -> None:
        ...
        self.folder_name = "inference_results"
        self.csv_name = "results.csv"
        
        if not os.path.isdir(self.folder_name):
            os.mkdir(self.folder_name)
            
        if not os.path.isfile(f"{self.folder_name}/{self.csv_name}"):
            f = open(f"{self.folder_name}/{self.csv_name}", 'w')
            f.close()
            
        header = ['Upper Color', 'Lower Color', 'Gender', 'Bag', 'Hat']
        
        with open(f'{self.folder_name}/{self.csv_name}', 'w', encoding='UTF8',newline='') as f:
            writer = csv.writer(f)
    ...
    def inference(self):
        ...
        for input, input_name in self.image_reader_generic():
            ...
            self.write_on_csv(results, input_name)
    ...
    def write_on_csv(self, results, image_name):
        
        with open(f'{self.folder_name}/{self.csv_name}', 'a', encoding='UTF8',newline='') as f:
            writer = csv.writer(f)
        
            result_string = [image_name + ": " + results["upper_color"], results["lower_color"], results["gender"], results["bag"], results["hat"]]
...
if __name__ == "__main__":
    ...
    images_path = "test_colors_on_PAR_dataset\\validation_set_reduced"
    
    inference_model = ModelInference(model_path=model_path, images_path=images_path)
